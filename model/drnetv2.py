...
import model.lib.resnet as models
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.load_pretrain_weight(layers, args.split)
    ...
    def load_pretrain_weight(self, layers=50, split=0):
        print('load weight from pretrained pspnet')
        weight_path = f'initmodel/pspnet/resnet{layers}/split{split}.pth'
        new_param = torch.load(weight_path, map_location=torch.device('cpu'))['state_dict']
        try:
            self.load_state_dict(new_param)
        except RuntimeError:
            model_dict = self.state_dict()
            new_dict = {k: v for k, v in new_param.items() if k in model_dict.keys()}
