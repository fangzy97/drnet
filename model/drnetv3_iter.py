...
def Weighted_GAP(supp_feat, mask):
    ...
    area = F.avg_pool2d(mask, (supp_feat.size()[2], supp_feat.size()[
                        3])) * feat_h * feat_w + 0.0005
    supp_feat = F.avg_pool2d(
        input=supp_feat, kernel_size=supp_feat.shape[-2:]) * feat_h * feat_w / area
    return supp_feat
...
class Model(nn.Module):
    ...
    def forward(self, x, s_x, s_y, y=None):
        ...
        with torch.no_grad():
            ...
            if self.vgg:
                query_feat_2 = F.interpolate(query_feat_2, size=(query_feat_3.size(
                    2), query_feat_3.size(3)), mode='bilinear', align_corners=True)
        ...
        for i in range(self.shot):
            ...
            with torch.no_grad():
                supp_feat_2, supp_feat_3, supp_feat_4 = self.extract_features(
                    s_x[:, i, :, :, :])
                if self.vgg:
                    supp_feat_2 = F.interpolate(supp_feat_2, size=(supp_feat_3.size(
                        2), supp_feat_3.size(3)), mode='bilinear', align_corners=True)
        ...
        for i, tmp_supp_feat in enumerate(final_supp_list):
            ...
            tmp_query_feat = query_feat_4
            for _ in range(self.iter_num):
                fg_init_map = torch.cosine_similarity(
                    tmp_query_feat, fg_vectors)
                bg_init_map = torch.cosine_similarity(
                    tmp_query_feat, bg_vectors)
                init_map = torch.stack((bg_init_map, fg_init_map), dim=1)
                init_mask = init_map.argmax(dim=1, keepdim=True)
                rec_vector = Weighted_GAP(tmp_query_feat, init_mask.float())
                rec_map = torch.cosine_similarity(tmp_query_feat, rec_vector)
                rec_map = self.normalization(rec_map)
                tmp_query_feat = tmp_query_feat * rec_map

            rec_map = F.interpolate(rec_map, size=(
                sz, sz), mode='bilinear', align_corners=True)
        ...
        for idx, tmp_bin in enumerate(self.pyramid_bins):
            ...
            cr_feat_bin = F.interpolate(cr_out, size=(
                bin, bin), mode='bilinear', align_corners=True)
            sr_out_bin = F.interpolate(sr_out, size=(
                bin, bin), mode='bilinear', align_corners=True)
            merge_feat_bin = torch.cat(
                [query_feat_bin, supp_feat_bin, sr_out_bin, cr_feat_bin], 1)
            merge_feat_bin = self.init_merge[idx](merge_feat_bin)

            if idx >= 1:
                pre_feat_bin = pyramid_feat_list[idx - 1].clone()
                pre_feat_bin = F.interpolate(pre_feat_bin, size=(
                    bin, bin), mode='bilinear', align_corners=True)
            ...
            merge_feat_bin = F.interpolate(merge_feat_bin, size=(query_feat.size(
                2), query_feat.size(3)), mode='bilinear', align_corners=True)
        ...
        if self.training:
            ...
            for idx_k in range(len(out_list)):
                ...
                inner_out = F.interpolate(inner_out, size=(
                    h, w), mode='bilinear', align_corners=True)
