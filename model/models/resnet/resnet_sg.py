...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes):
        ...
        if self.hehe:
            self.residule4 = nn.Sequential(
                nn.ReLU(),
                nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=True),
                nn.ReLU(),
                nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=True),
                nn.ReLU(),
                nn.Conv2d(256, 2, kernel_size=3, stride=1, padding=1, bias=True),
            )
    ...
    def forward(self, query_rgb,support_rgb,support_mask,history_mask):
        ...
        #fb_mask = self.residule4(query_rgb)
        #fb_mask = self.softmax(fb_mask)
        ...
        #fb_mask=F.interpolate(fb_mask,feature_size,mode='bilinear',align_corners=True)
