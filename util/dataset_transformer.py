...
def make_dataset(split=0, data_root=None, data_list=None, sub_list=None):
    ...
    split_data_list = data_list.split(
        '.')[0] + '_split{}'.format(split) + '.pth'
...
class SemData(Dataset):
    def __init__(self, split=3, shot=1, data_root=None, data_list=None, transform=None, mode='train', use_coco=False, use_split_coco=False):
        ...
        if not use_coco:
            ...
            if use_split_coco:
                ...
                if self.split == 3:
                    ...
                    self.sub_list = list(
                        set(self.class_list) - set(self.sub_val_list))
                elif self.split == 2:
        ...
        if self.mode == 'train':
            self.data_list, self.sub_class_file_list = make_dataset(
                split, data_root, data_list, self.sub_list)
            assert len(self.sub_class_file_list.keys()) == len(self.sub_list)
        elif self.mode == 'val':
            self.data_list, self.sub_class_file_list = make_dataset(
                split, data_root, data_list, self.sub_val_list)
            assert len(self.sub_class_file_list.keys()
                       ) == len(self.sub_val_list)
    ...
    def __getitem__(self, index):
        ...
        assert len(support_label_list) == self.shot and len(
            support_image_list) == self.shot

        raw_label = label.copy()
        if self.transform is not None:
            image, label, padding_mask = self.transform(
                image, label, padding_mask)
            for k in range(self.shot):
