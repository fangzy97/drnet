...
def poly_learning_rate_ori(optimizer, base_lr, curr_iter, max_iter, power=0.9, index_split=-1, scale_lr=10., warmup=False, warmup_step=500):
    """poly learning rate policy"""
    if warmup and curr_iter < warmup_step:
        lr = base_lr * (0.1 + 0.9 * (curr_iter/warmup_step))
    else:
...
def Metrics(output, target, K, ignore_index=255):
    # 'K' classes, output and target sizes are N or N * L or N * H * W, each value in range 0 to K - 1.
    assert (output.dim() in [1, 2, 3])
    assert output.shape == target.shape
    output = output.view(-1)
    ...
    # print(area_union)
    # exit()
    s_TP = area_intersection[1]
    s_FP = area_output[1] - s_TP
    s_FN = area_target[1] - s_TP
    s_TN = area_intersection[0]

    precision = s_TP / (s_TP + s_FP + 1e-10)
    recall = s_TP / (s_TP + s_FN + 1e-10)
    specificity = s_TN / (s_TN + s_FP + 1e-10)
