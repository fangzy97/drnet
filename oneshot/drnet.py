...
class OneModel(nn.Module):
    ...
    def forward(self, anchor_img, pos_img, neg_img, pos_mask, category=0, group=None):
        ...
        outA_pos = F.upsample(outA, size=(mask_w, mask_h), mode='bilinear', align_corners=True)
        ...
        tmp_neg = self.cos_similarity_func(outB, vec_n)  # Prototypes BG
    ...
    def get_loss(self, logits, query_label, fb_mask=None):
    ...
    def get_pred(self, logits, query_image):
        ...
        outB_side = F.upsample(outB_side, size=(w, h), mode='bilinear')  # Recalibrated Probability Map
        vec_pos = F.upsample(vec_pos, size=(w, h), mode='bilinear')  # Initial Probability Map
        out_softmax = F.softmax(0.4*vec_pos+outB_side, dim=1).squeeze()
        values, pred = torch.max(out_softmax, dim=0)

        out_softmax1 = F.softmax(vec_pos, dim=1).squeeze()
        values1, pred1 = torch.max(out_softmax1, dim=0)

        out_softmax2 = F.softmax(outB_side, dim=1).squeeze()
        values2, pred2 = torch.max(out_softmax2, dim=0)
        # print(pred2.size())
        return 0.4*vec_pos+outB_side, pred
