...
class mydataset(Dataset):
    ...
    def __init__(self, args, is_train=True, transform=None, aug=None):
        """
        Args:
    ...
    def get_item_mlclass_train(self, query_img, support_img, category):
        ...
        neg_img, neg_mask = self._read_mlclass_train1(query_img, category)
    ...
    def get_item_diva_train(self, query_img, support_img, sample_list, category):
        # query_img, support_img, [icf_ap, icf_an, cdf_an, csf_ap]
        que_img, que_mask = self._read_mlclass_train(query_img, category[0])
        supp_img, supp_mask = self._read_mlclass_train(support_img, category[0])
        icf_ap, icf_ap_mask = self._read_mlclass_train(sample_list[0], category[0])
        icf_an, icf_an_mask = self._read_mlclass_train(sample_list[1], category[0])

        cdf_an, cdf_an_mask = self._read_mlclass_train(sample_list[2], category[1])
        csf_ap, csf_ap_mask = self._read_mlclass_train(sample_list[3], category[2])
        if self.transform is not None:
            # add augmentation here
            que_img = Image.fromarray(np.uint8(que_img))
            supp_img = Image.fromarray(np.uint8(supp_img))
            icf_ap = Image.fromarray(np.uint8(icf_ap))
            icf_an = Image.fromarray(np.uint8(icf_an))
            cdf_an = Image.fromarray(np.uint8(cdf_an))
            csf_ap = Image.fromarray(np.uint8(csf_ap))

            que_img = self.transform(que_img)
            supp_img = self.transform(supp_img)
            icf_ap = self.transform(icf_ap)
            icf_an = self.transform(icf_an)
            cdf_an = self.transform(cdf_an)
            csf_ap = self.transform(csf_ap)
            ssf_an = self.aug(que_img)

            que_mask = cv2.resize(que_mask, (321,321), interpolation=cv2.INTER_NEAREST)
            supp_mask = cv2.resize(supp_mask, (321,321), interpolation=cv2.INTER_NEAREST)
            icf_ap_mask = cv2.resize(icf_ap_mask, (321,321), interpolation=cv2.INTER_NEAREST)
            icf_an_mask = cv2.resize(icf_an_mask, (321, 321), interpolation=cv2.INTER_NEAREST)
            cdf_an_mask = cv2.resize(cdf_an_mask, (321, 321), interpolation=cv2.INTER_NEAREST)
            csf_ap_mask = cv2.resize(csf_ap_mask, (321, 321), interpolation=cv2.INTER_NEAREST)
            ssf_an_mask = cv2.flip(que_mask, 1)


        # cdf_a, cdf_a_mask = supp_img, supp_mask
        # csf_a, csf_a_mask = supp_img, supp_mask
        # icf_a, icf_a_mask = supp_img, supp_mask
        #
        # cdf_ap, cdf_ap_mask = que_img, que_mask
        # ssf_a, ssf_a_mask = que_img, que_mask
        return que_img, que_mask, supp_img, supp_mask, [icf_ap, icf_an, cdf_an, csf_ap, ssf_an], [icf_ap_mask, icf_an_mask, cdf_an_mask, csf_ap_mask, ssf_an_mask], category+1
    ...
    def __getitem__(self, idx):
        ...
        elif self.split == 'diva_train':
            query_img, support_img, sample_list, category = self.img_db.get_diva_train(split='train', group=self.group, num_folds=4)

            return self.get_item_diva_train(query_img, support_img, sample_list, category)
