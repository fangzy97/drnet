...
class PASCAL:
    ...
    def create_anns(self, read_mode):
        with open(osp.join(self.db_path, 'ImageSets', 'Segmentation', 'train_hc' + '.txt'), 'r') as f:
            lines = f.readlines()    
            names = []
            for line in lines:
                if line.endswith('\n'):
        ...
        for item in names:
            ...
            mobj_path = osp.join(self.db_path, 'SegmentationClassAug', item + '.png')
    ...
    def getItems(self, cats=[], areaRng=[], read_mode = PASCAL_READ_MODES.INSTANCE):
        ...
        for i in range(len(anns)):
            ...
            scribble_path = osp.join(self.db_path, 'ScribbleAugAuto', ann['mask_name'] + '.png')
            if read_mode == PASCAL_READ_MODES.INSTANCE:
                mask_path = osp.join(self.db_path, 'SegmentationObject', ann['mask_name'] + '.png')
                item = DBPascalItem('pascal-'  + self.dataType + '_' + ann['image_name'] + '_' + str(i), img_path, mask_path, scribble_path, ann['object_ids'])
            else:
                mask_path = osp.join(self.db_path, 'SegmentationClassAug', ann['mask_name'] + '.png')
                item = DBPascalItem('pascal-'  + self.dataType + '_' + ann['image_name'] + '_' + str(i), img_path, mask_path, scribble_path, ann['class_ids'], ids_map)
            items.append(item)
        return items
...
class DBPascalItem(DBImageItem):
    def __init__(self, name, img_path, mask_path, scribble_path, obj_ids, ids_map = None):
        DBImageItem.__init__(self, name)
    ...
    def read_mask(self, orig_mask=False):
        ...
        mobj_uint_scribble = np.array(Image.open(self.scribble_path))

        if orig_mask:
            return mobj_uint.astype(np.float32), mobj_uint_scribble.astype(np.float32)
