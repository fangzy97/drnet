...
def get_arguments():
    ...
    parser.add_argument("--arch", type=str,default='OPM9')
    ...
    for group in range(4):
        ...
        for dat in train_loader:
            ...
            for i in range(5):
                ...
                pos = get_org_img(pos_img.squeeze().cpu().data.numpy())
                cv2.imwrite(str(i)+'.png', pos)
                pos_mask = supp_mask[i].cuda()
                pos_mask[pos_mask>0.] = 1.
                pos_mask = torch.unsqueeze(pos_mask, dim=1)


                logits = model(que_img, pos_img, None, pos_mask, mode=0)
