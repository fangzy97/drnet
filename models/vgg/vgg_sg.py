...
class VGG(nn.Module):
    ...
    def __init__(self, features, use_decoder=False, use_border=False):
        super(VGG, self).__init__()
        self.features_dict = features

        self.decoder = use_decoder
        self.border = use_border
        ...
        if self.decoder and self.border:
            self.border = nn.Sequential(
                nn.ReLU(),
                nn.Conv2d(128, 64, kernel_size=3, dilation=1, padding=1), # border convolution
            )
    ...
    def forward(self, x):
        ...
        if self.decoder:
            ...
            if self.border:
                border = self.border(side_out5_1)

                return out, side_out5_1, border
