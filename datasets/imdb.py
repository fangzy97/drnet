...
class imdb(object):
    ...
    def get_triple_images(self, split, group, num_folds=4):
        ...
        return (anchor_img, pos_img, neg_img, rand_cat[0]+1)
    ...
    def get_diva_train(self, group, num_folds=4):
        cats = self.get_cats('train', group, num_folds)
        rand_cat = np.random.choice(cats, 3, replace=False)[0]
        cat_list = self.multiclass_grouped_imgs[rand_cat[0]]
        sample_img_ids_1 = np.random.choice(len(cat_list), 4, replace=False)
        query_img = cat_list[sample_img_ids_1[0]]
        support_img = cat_list[sample_img_ids_1[1]]
        icf_ap = cat_list[sample_img_ids_1[2]]
        icf_an = cat_list[sample_img_ids_1[3]]


        # attain class_discriminative features
        cat_list = self.multiclass_grouped_imgs[rand_cat[1]]
        sample_img_ids_2 = np.random.choice(len(cat_list), 1, replace=False)
        cdf_an = cat_list[sample_img_ids_2]
        # csf_an = cat_list[sample_img_ids_2]

        cat_list = self.multiclass_grouped_imgs[rand_cat[2]]
        sample_img_ids_3 = np.random.choice(len(cat_list), 1, replace=False)
        csf_ap = cat_list[sample_img_ids_3]

        return query_img, support_img, [icf_ap, icf_an, cdf_an, csf_ap], rand_cat
